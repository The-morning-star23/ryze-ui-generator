...
// 2. Card - Updated with semantic status colors for titles
...
// 4. Navbar - Robust label handling
...
// 5. Table - Defensive rendering for headers and rows
    ...
    <div className="overflow-hidden border border-gray-100 rounded-xl shadow-sm">
      <table className="w-full text-sm text-left">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>
            {safeHeaders.map((h, i) => (
              <th key={i} className="px-4 py-3 font-bold text-gray-600 uppercase text-[10px] tracking-widest">{h}</th>
            ))}
          </tr>
        </thead>
            ...
            <tr key={i} className="bg-white hover:bg-blue-50/30 transition-colors">
              {(Array.isArray(row) ? row : []).map((cell, j) => (
                <td key={j} className="px-4 py-3 text-gray-600">
                  {typeof cell === 'object' && cell !== null ? (cell.label || JSON.stringify(cell)) : cell}
                </td>
              ))}
            </tr>
          ))}
...
// 6. Sidebar - Defensive layout wrapper
export const Sidebar = ({ items = [], children }) => {
  const safeItems = Array.isArray(items) ? items : [];

  return (
    <div className="flex w-full border border-gray-200 rounded-2xl overflow-hidden min-h-[500px] bg-white shadow-inner">
      <div className="w-48 bg-gray-50 border-r border-gray-100 p-4 space-y-1 flex-shrink-0">
        ...
        {safeItems.map((item, i) => {
          const label = typeof item === 'object' && item !== null ? (item.label || item.text) : item;
          return (
...
export const Chart = ({ title, type = "Performance" }) => (
  <div className="border border-gray-100 rounded-xl p-5 bg-white shadow-sm mb-4">
    <div className="flex justify-between items-center mb-6">
      <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest">{title || 'Data Chart'}</h4>
      <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold uppercase">{type}</span>
    </div>
    <div className="h-32 w-full flex items-end justify-around gap-2 px-2">
      {[50, 80, 40, 95, 60, 85, 30].map((h, i) => (
        <div key={i} style={{ height: `${h}%` }} className="w-full bg-gradient-to-t from-blue-600 to-blue-400 rounded-t-md opacity-90 hover:opacity-100 transition-opacity" />
      ))}
    </div>
  </div>
);

// 8. Modal - Robust child rendering
export const Modal = ({ title, children, isOpen = true }) => {
  ...
  return (
    <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-6 animate-in fade-in duration-300">
      <div className="bg-white rounded-2xl max-w-sm w-full p-8 shadow-2xl border border-white/20">
        ...
        <div className="text-sm text-gray-500 leading-relaxed mb-6">
          {typeof children === 'object' && children !== null ? (children.text || children.label || JSON.stringify(children)) : children}
        </div>
        <button className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-sm shadow-lg hover:bg-blue-700 transition-all">
          Confirm & Close
        </button>
      </div>
    </div>
